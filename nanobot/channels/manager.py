...
class ChannelManager:
    ...
    async def stop_for_cron_job(self, drain_timeout: float = 15.0) -> None:
        ...
        Waits for the outbound queue to drain so messages can be delivered
        ...
        for name, channel in self.channels.items():
            try:
                await channel.stop()
                logger.info("Stopped {} channel after cron", name)
            except Exception as e:
                logger.error("Error stopping {} after cron: {}", name, e)
