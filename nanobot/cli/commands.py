...
# ============================================================================
# Cron Commands (orghi: workspace-scoped store; `run` uses gateway-equivalent agent)
# ============================================================================
...
def _cron_jobs_file(config: Config) -> Path:
...
def cron_list(
    ...
    service = CronService(_cron_jobs_file(config))
...
def cron_add(
    ...
    channel: str = typer.Option(
        None,
        "--channel",
        help="Channel for delivery (e.g. 'telegram', 'whatsapp')",
    ),
):
    """Add a scheduled job."""
    from nanobot.config.loader import load_config
...
def cron_run(
    ...
    job = next(
        (j for j in service.list_jobs(include_disabled=True) if j.id == job_id),
        None,
    )
    if not job:
        console.print(f"[red]Job {job_id} not found[/red]")
    ...
    async def on_job(cj: CronJob) -> str | None:
        from nanobot.agent.tools.cron import CronTool
        from nanobot.agent.tools.message import MessageTool
        from nanobot.bus.events import OutboundMessage
        from nanobot.utils.evaluator import evaluate_response
        ...
        reminder_note = (
            "[Scheduled Task] Timer finished.\n\n"
            f"Task '{cj.name}' has been triggered.\n"
            f"Scheduled instruction: {cj.payload.message}"
        )
        ...
        try:
            resp = await agent.process_direct(
                reminder_note,
                session_key=f"cron:{cj.id}",
                channel=cj.payload.channel or "cli",
                chat_id=cj.payload.to or "direct",
            )
        finally:
            if isinstance(cron_tool, CronTool) and cron_token is not None:
                cron_tool.reset_cron_context(cron_token)
        ...
        if cj.payload.deliver and cj.payload.to and response:
            should_notify = await evaluate_response(
                response,
                cj.payload.message,
                provider,
    ...
    async def run_cron() -> bool:
        ...
        if needs_delivery:
            channels = ChannelManager(config, bus)
            if channels.enabled_channels:
                await channels.start_for_cron_job()
            else:
                logger.warning("Job requires delivery but no channels enabled")

        ok = await service.run_job(job_id, force=force)

        try:
            if channels and channels.enabled_channels:
                await channels.stop_for_cron_job()
        finally:
            await agent.close_mcp()

        return ok

    if asyncio.run(run_cron()):
        console.print("[green]✓[/green] Job executed")
        if last_response:
            _print_agent_response(last_response[-1], render_markdown=True)
    else:
